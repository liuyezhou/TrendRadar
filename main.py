...
def send_to_notifications(
    ...
    # 发送到飞书外部群（多账号）
    feishu_urls = parse_multi_account_config(CONFIG["FEISHU_OUTSIDE_WEBHOOK_URL"])
    if feishu_urls:
        feishu_urls = limit_accounts(feishu_urls, max_accounts, "飞书")
        feishu_results = []
        for i, url in enumerate(feishu_urls):
            if url:  # 跳过空值
                account_label = f"账号{i+1}" if len(feishu_urls) > 1 else ""
                result = send_to_feishu(
                    url, report_data, report_type, update_info_to_send, proxy_url, mode, account_label, True
                )
                feishu_results.append(result)
        results["feishu_outside"] = any(feishu_results) if feishu_results else False
